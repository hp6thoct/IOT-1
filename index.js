...
app.get("/", async (req, res) => {
  ...
  // Get state of light
  const lightStateResponse = await getSheetValues(
    auth,
    spreadsheetId,
    "Iot!C2:C2"
  );
  ...
  // Get state of Door
  const doorStateResponse = await getSheetValues(
    auth,
    spreadsheetId,
    "Iot!B2:B2"
  );
...
app.post("/login", async (req, res) => {
  ...
  const authSuccessful = accountData.some(
